...
function buildTreeData(nodes: GenreTreeNode[]) {
  ...
  const { highestVerticalCoordinate, svgWidth, svgHeight } =
    calculateSvgDimensions(d3, laidOut);
...
function baseCallbacks(
  overrides: Partial<RenderTreeCallbacks> = {},
): RenderTreeCallbacks {
  return {
...
describe("calculateSvgDimensions / setupTreeLayout / createTreeLayout", () => {
  ...
  it("reserves at least a full node's height for a single, childless root", () => {
    const root = buildTreeHierarchyStructure(d3, [
      { id: "root", parentId: null, name: "Root", itemCount: 0 },
    ]);
    const laidOut = createTreeLayout(d3, root);
    const dims = calculateSvgDimensions(d3, laidOut);
    expect(dims.svgHeight).toBeGreaterThanOrEqual(
      MAX_NODE_HEIGHT + ACTIONS_OVERLAY_HEIGHT,
    );
  });
...
describe("calculateSvgDimensions / setupTreeLayout / createTreeLayout (vertical orientation)", () => {
  ...
  it("centers the root's own visual anchor (x + own width / 2) at svgWidth / 2 and grows y upward with depth", () => {
    const root = buildTreeHierarchyStructure(d3, VERTICAL_NODES);
    const laidOut = createTreeLayout(d3, root, "vertical");
    const { svgWidth, highestVerticalCoordinate } = calculateSvgDimensions(
      d3,
      laidOut,
      "vertical",
    );
    ...
    const itemCountRange = getItemCountRange(
      treeData.descendants().map((d) => d.data),
    );
    const rootWidth = calculateNodeDimensions(
      treeData.data.itemCount,
      itemCountRange,
    ).WIDTH;
    ...
    const deepestNode = descendants.reduce((deepest, d) =>
...
describe("calculateSvgDimensions / setupTreeLayout / createTreeLayout (horizontal-anchored orientation)", () => {
  ...
  it("centers the root's own visual anchor (y + own height / 2) at svgHeight / 2 and grows x rightward with depth", () => {
    const root = buildTreeHierarchyStructure(d3, HORIZONTAL_ANCHORED_NODES);
    const laidOut = createTreeLayout(d3, root, "horizontal-anchored");
    const { svgHeight, highestVerticalCoordinate } = calculateSvgDimensions(
      d3,
      laidOut,
      "horizontal-anchored",
    );
    ...
    const rootHeight = calculateNodeDimensions(
      treeData.data.itemCount,
      itemCountRange,
    ).HEIGHT;
  ...
  it("keeps depth-to-depth center spacing uniform when the root is hidden", () => {
    ...
    const laidOut = createTreeLayout(d3, root, "horizontal-anchored");
    const { highestVerticalCoordinate, rootDepthOffset } =
      calculateSvgDimensions(d3, laidOut, "horizontal-anchored", true);
    const treeData = setupTreeLayout(
      d3,
      laidOut,
      highestVerticalCoordinate,
    ...
    treeData.each((d) => {
      const width = calculateNodeDimensions(
        d.data.itemCount,
        itemCountRange,
      ).WIDTH;
...
describe("calculateSvgDimensions / setupTreeLayout / createTreeLayout (horizontal-anchored-flipped orientation)", () => {
  ...
  it("computes positive svg dimensions for a small tree", () => {
    const root = buildTreeHierarchyStructure(
      d3,
      HORIZONTAL_ANCHORED_FLIPPED_NODES,
    );
    const laidOut = createTreeLayout(d3, root, "horizontal-anchored-flipped");
    const dims = calculateSvgDimensions(
      d3,
      laidOut,
  ...
  it("centers the root's own visual anchor (y + own height / 2) at svgHeight / 2 and grows x leftward with depth", () => {
    ...
    const laidOut = createTreeLayout(d3, root, "horizontal-anchored-flipped");
    const { svgWidth, svgHeight, highestVerticalCoordinate } =
      calculateSvgDimensions(d3, laidOut, "horizontal-anchored-flipped");
    const treeData = setupTreeLayout(
      d3,
      laidOut,
      highestVerticalCoordinate,
  ...
  it("keeps depth-to-depth center spacing uniform when the root is hidden", () => {
    ...
    const root = buildTreeHierarchyStructure(d3, LINEAR_NODES);
    const laidOut = createTreeLayout(d3, root, "horizontal-anchored-flipped");
    const { svgWidth, highestVerticalCoordinate, rootDepthOffset } =
      calculateSvgDimensions(d3, laidOut, "horizontal-anchored-flipped", true);
    const treeData = setupTreeLayout(
      d3,
      laidOut,
      highestVerticalCoordinate,
...
describe("renderTree", () => {
  ...
  it("does not scope a shadow filter id while ELEVATION is disabled, even for unsafe-character ids", () => {
    const nodes: GenreTreeNode[] = [
      { id: "root id/with:chars", parentId: null, name: "Root", itemCount: 0 },
    ];
    ...
    expect(
      svgEl.querySelector("filter#gtv-card-shadow-root_id_with_chars"),
    ).toBeFalsy();
  });
  ...
  it("gives the selected node an accent border and dims every other node", () => {
    ...
    const rootBorder = rootGroup.querySelector(
      ".gtv-node-border",
    ) as SVGPathElement;
    const childBorder = childGroup.querySelector(
      ".gtv-node-border",
    ) as SVGPathElement;
  ...
  it("dims only links not touching the selected node, leaving its parent-link and children-links undimmed", () => {
    const nodes: GenreTreeNode[] = [
      { id: "root", parentId: null, name: "Root", itemCount: 5 },
    ...
    const links = Array.from(svgEl.querySelectorAll("path.gtv-link"));
    const dimmedCount = links.filter((l) =>
      l.getAttribute("class")?.includes("gtv-link--dimmed"),
    ).length;
    const undimmedCount = links.length - dimmedCount;

    expect(links.length).toBe(3);
    expect(undimmedCount).toBe(2);
  ...
  it("renders the label as just the node name, with no item count suffix", () => {
    ...
    const rootLabel = svgEl.querySelector(
      "#group-root .gtv-node-label",
    ) as HTMLDivElement;
    const childLabel = svgEl.querySelector(
      "#group-child .gtv-node-label",
    ) as HTMLDivElement;
  ...
  it("does not add a toolbar on mouseover while a reparent is in progress or the node is forbidden", () => {
    ...
    const childForeignObject = svgEl.querySelector(
      "#group-child foreignObject",
    ) as SVGForeignObjectElement;
    childForeignObject.dispatchEvent(
  ...
  it("adds the hover-name label on mouseover alongside the toolbar", () => {
    ...
    const rootForeignObject = svgEl.querySelector(
      "#group-root foreignObject",
    ) as SVGForeignObjectElement;
    rootForeignObject.dispatchEvent(
      new MouseEvent("mouseover", { bubbles: true }),
    );

    expect(
      svgEl.querySelector("#hover-label-root .gtv-hover-label")!.textContent,
    ).toBe("Root");
  });
  ...
  it("adds the reparent-target overlay on mouseenter only for eligible, non-forbidden nodes", () => {
    ...
    expect(
      svgEl.querySelector("#select-as-new-parent-group-child"),
    ).toBeTruthy();
  });
  ...
  it("keeps the toolbar and hover-name label mounted on the mouseleave timeout while the overflow menu is open", () => {
    ...
    const kebab = svgEl.querySelector(
      '#toolbar-root [data-menu-key="__more"]',
  ...
  it("also removes a lingering reparent-target overlay on the same mouseleave timeout", () => {
    ...
    expect(
      svgEl.querySelector("#select-as-new-parent-group-root"),
    ).toBeTruthy();
