...
describe("GenreTreeWheel", () => {
  ...
  it("reserves anchor clearance from the selected root's own local subtree size, not the cross-root chip scale", () => {
    ...
    // cross-root scale used to size the chips, root-a's chip would be near MIN_NODE_HEIGHT, but the
    ...
    const expectedHalfExtent = calculateLocalRootDimensions(d3, [nodes[0], nodes[1]]).HEIGHT / 2;
