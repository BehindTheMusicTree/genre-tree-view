...
export function WheelCore({
  ...
  // consecutive-depth gap. See calculateRootAnchorClearance's doc comment (and
  // GenreTreeWheelRadialBase for the equivalent radial-wheel computation) for why that offset needs
  // both the hidden root's own local half-extent and the visible chip's cross-root display
  // half-extent, not just one or the other.
  const rootChipHalfExtent = selectedGroup
    ? calculateRootAnchorClearance(
        d3,
        selectedGroup.nodes,
        aggregatedRootItemCountById.get(selectedGroup.root.id)!,
        rootItemCountRange,
        direction === "left" ? "WIDTH" : "HEIGHT",
      )
    : 0;
